...
module.exports = async (req, res) => {
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "dkkeys22@gmail.com";
  const from = process.env.CONTACT_FROM_EMAIL || "DK Keys <leads@dkkeys.com>";

  const subject = `New lead from dkkeys.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on dkkeys.com`;
  const html = `
<h2>New lead from DK Keys</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on dkkeys.com</p>`;
